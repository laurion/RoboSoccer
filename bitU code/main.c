...
void fwd_backwrd()
{
    if(coordrob[portar].angle < 270 && coordrob[portar].angle > 90)
    {
        crt[portar].right = -60;
        ctr[portar].left = -60;
        ctr[portar].time = timpmiscareportar/2;
    ...
    if(coordrob[portar].angle > 10 && (360 - coordrob[portar].angle))
       {
        crt[portar].right = -50;
        ctr[portar].left = 50;
        ctr[portar].time = timpmiscareportar/2;
...
void atancant_cadrane(int robot)
{
    if((cadran(coordrob[robot].x , coordrob[robot].y) == 1)&&(cadran(coordrob[0].x,coordrob[0].y) == 1))
    {
        if()
    }
}
